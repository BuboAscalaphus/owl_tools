...
# Nomi univoci per non andare in conflitto con l'altro script
PID_FILE=/tmp/rtk.pid
PGID_FILE=/tmp/rtk.pgid
LOG_FILE=/home/dev/bags/rtk.log

PKG_NAME="owl_rtk"
# Assumo che il file sia dentro la cartella "launch/" del pacchetto
LAUNCH_REL="launch/rtk.launch.py" 
...
start_bg() {
  docker exec \
    ...
    "$NAME" bash -lc '
      ...
      # --- MODIFICA QUI ---
      # Forza Python a scrivere immediatamente su file senza buffer
      export PYTHONUNBUFFERED=1
      # Forza ROS2 a usare lo stream stdout in modo bufferizzato per linea (o unbuffered)
  ...
  docker exec \
    ...
    "$NAME" bash -lc '
      ...
      if [[ -n "$target" ]]; then
        ...
        # Fallback specifico per rtk
        pkill -INT -f "ros2 launch .*owl_rtk.*" 2>/dev/null || true
      fi

      rm -f "$PID_FILE" "$PGID_FILE" || true
      pkill -TERM -f "ros2 launch .*owl_rtk.*" 2>/dev/null || true
    '
}
...
status_bg() {
  docker exec \
    ...
    "$NAME" bash -lc '
      ...
      running=0
      if [[ -f "$PID_FILE" ]] && kill -0 "$(cat "$PID_FILE")" 2>/dev/null; then
        echo "✅ RTK running (pid $(cat "$PID_FILE"))"
        running=1
      elif pgrep -af "ros2 launch .*owl_rtk.*" >/dev/null; then
        echo "⚠️ RTK seems running (found by pattern), but PID file is missing."
        running=1
      else
        echo "no RTK running"
      fi
...
case "${1:-}" in
  start)
    ...
    if [[ "$out" == "already_running" ]]; then
      echo "ℹ️ RTK already running (see: $0 status / $0 tail)"
    else
      echo "✅ RTK started (logging to $LOG_FILE)"
    fi
    ;;
  stop)
    require_running_container
